...
function endpoints(state=defaultState, action) {
    let list;
    switch (action.type) {
    ...
    case types.RECEIVE_ENDPOINTS:
        return Object.assign({}, state, {
            ...
            visibleList: action.endpoints,
        });

    case types.FILTER_ENDPOINT_ON_TYPE:
        ...
        list = action.types.map((type) => {
            return state.list.filter((endpoint) => {
