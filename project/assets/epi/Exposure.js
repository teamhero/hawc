...
class Exposure {
    ...
    build_central_tendencies_table() {
        ...
        var ctHeaders = ["Estimate", "Estimate type", "Variance", "Variance type",
                        "Lower Bound Interval", "Upper Bound Interval", "Lower CI", "Upper CI", "Lower Range", "Upper Range" ];

        var headerRow = $("<tr/>").appendTo(ctTable);
        for (var i = 0 ; i < ctHeaders.length ; i++) {
            $("<th/>").html(ctHeaders[i].replace(/ /g, "&nbsp")).appendTo(headerRow);
        }
