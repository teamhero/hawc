...
class AssessmentPermissionsMixin(object):
    ...
    # new method - tries to determine the relevant study for a given operation,
    # and then checks its editable status. If not set, raises a PermissionDenied.
    def deny_for_locked_study(self, user, assessment, obj):
        # print("#" * 100)
        # print("fetching study for class [" + str(type(self)) + "] with model [" + str(self.model) + "]")
        # print("relevant instance is [" + str(type(obj)) + "]: [" + str(obj) + "]")
...
class CopyAsNewSelectorMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        # prevents users from doing a "Copy as New" / "Copy from Existing" etc. on a study that has just been locked.
